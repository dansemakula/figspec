...
#' recorded in a specification. The specification may come from a publisher,
#' a journal or a project. Every
...
#' [fig_save()] reads that width from the specification and writes the finished
#' file accordingly.
#'
...
#'   Visual choices are applied first, followed by any overlapping requirements
#'   in the specification.
#' @param base_size Base type size in points. Defaults to the specification's
#'   stated minimum, or 9 pt when none is recorded.
#' @param base_family Font family. Defaults to the graphics device's own font.
#'   [fig_save()] resolves a font named by the specification when it selects the
#'   output device. Pass a family here when you already know which installed
#'   font the current device should use.
