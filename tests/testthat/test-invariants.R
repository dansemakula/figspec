...
test_that("a hedge about the document does not downgrade a rule inside it", {
  ...
  expect_equal(spec_get("oup")$dpi_min, 300)
...
test_that("a hedge about achievability does not downgrade a rule", {
  ...
  expect_equal(spec_get("elsevier")$font_min_pt, 6)
  expect_match(spec_get("elsevier")$notes, "rule-of-thumb")
})
...
test_that("a list is recorded only where the publisher closes it", {
  ...
  expect_null(spec_get("springer")$font_families)
})
...
test_that("the README reports the registry size and links to its generated table", {
  ...
  # The full table now lives on the generated profiles page so the home page
  # stays readable. Keep its summary count and route to that table in sync.
