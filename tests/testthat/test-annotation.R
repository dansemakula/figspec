...
test_that("PNAS states the axis rule and is checked against it", {
  scatter <- ggplot2::ggplot(mtcars, ggplot2::aes(wt, mpg)) + ggplot2::geom_point()
  r <- fig_check(scatter, "pnas", column = "medium")
  ...
  expect_equal(fig_check(fixed, "pnas", column = "medium")[
    fig_check(fixed, "pnas", column = "medium")$check == "Axis origin", ]$status, "pass")
})

test_that("journals that state no axis rule raise no axis check", {
  scatter <- ggplot2::ggplot(mtcars, ggplot2::aes(wt, mpg)) + ggplot2::geom_point()
  expect_false("Axis origin" %in% fig_check(scatter, "plos_one")$check)
  expect_false("Axis origin" %in% fig_check(scatter, "cell_press")$check)
})
...
test_that("RSC's four-part limit is enforced", {
  ...
  expect_equal(fig_check(p + ggplot2::facet_wrap(~carb), "rsc_books")[
    fig_check(p + ggplot2::facet_wrap(~carb), "rsc_books")$check == "Panel count", ]$status,
    "fail")
})

test_that("journals that state no panel limit raise no panel-count check", {
  expect_false("Panel count" %in% fig_check(base_plot(), "plos_one")$check)
})
...
test_that("coloured text fails Nature, and grey text does not", {
  ...
  expect_equal(fig_check(red, "nature")[
    fig_check(red, "nature")$check == "Text colour", ]$status, "fail")

  # A grey has equal channels, so it is not a hue.
  grey <- base + ggplot2::theme(text = ggplot2::element_text(colour = "grey30"))
  expect_equal(fig_check(grey, "nature")[
    fig_check(grey, "nature")$check == "Text colour", ]$status, "pass")
...
test_that("Nature labels panels in lower case, like AGU and unlike Cell Press", {
  ...
  lower <- fig_check(comp + patchwork::plot_annotation(tag_levels = "a"), "nature")
  expect_equal(lower[lower$check == "Panel labels", ]$status, "pass")
  upper <- fig_check(comp + patchwork::plot_annotation(tag_levels = "A"), "nature")
...
test_that("a comma thousands separator fails the Royal Society's number rule", {
  ...
  r <- fig_check(commas, "royal_society")
  ...
  expect_equal(fig_check(spaced, "royal_society")[
    fig_check(spaced, "royal_society")$check == "Number format", ]$status, "pass")
})
...
test_that("only journals stating a number rule are checked against one", {
  ...
  expect_false("Number format" %in% fig_check(commas, "plos_one")$check)
})
...
test_that("BMJ's no-grid rule is scoped to bars, as BMJ scopes it", {
  ...
  r <- fig_check(hist_default, "bmj")
  expect_equal(r[r$check == "Background grid", ]$status, "fail")

  no_grid <- hist_default + ggplot2::theme(panel.grid = ggplot2::element_blank())
  expect_equal(fig_check(no_grid, "bmj")[
    fig_check(no_grid, "bmj")$check == "Background grid", ]$status, "pass")
