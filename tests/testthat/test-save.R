...
test_that("a saved TIFF comes back at the journal's width and resolution", {
  ...
  suppressWarnings(fig_save(path, plot_for_save("plos_one"),
                                  spec = font_neutral_spec("plos_one"),
...
test_that("saving in a format the journal does not accept warns", {
  ...
  expect_warning(
    fig_save(path, plot_for_save("plos_one"),
             spec = font_neutral_spec("plos_one"), check = FALSE),
    "accepted formats"
  )
})
...
test_that("a missing extension takes the journal's first accepted format", {
  ...
  suppressWarnings(fig_save(stem, plot_for_save("plos_one"),
                                  spec = font_neutral_spec("plos_one"),
                                  check = FALSE))
...
test_that("a default format is one R can actually write", {
  ...
  nature <- font_neutral_spec("nature")
  fmts <- tolower(unlist(nature$formats))
  expect_equal(fmts[[1]], "ai")
  expect_equal(default_format(nature), "eps")
...
test_that("fig_refit picks a writable format too", {
  ...
  res <- suppressWarnings(suppressMessages(
    fig_refit(list(fig1 = p), font_neutral_spec("nature"), dir)
  ))
  expect_equal(res$file, "fig1.eps")
})

test_that("a required font that is unavailable stops the export", {
  path <- withr::local_tempfile(fileext = ".tiff")
  testthat::local_mocked_bindings(
    resolve_family = function(families) "",
    device_resolves_system_fonts = function(ext) TRUE
  )
