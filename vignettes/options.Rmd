...
| `base_family` | Font family. Defaults to the graphics device's own font. The journal's named font is not forced here, because a family the current device cannot resolve makes the plot fail to render at all. ggsave_journal() applies the journal's font at save time, where the device is known. Pass a family explicitly to override. |
...
| Option | What it does |
|---|---|
| `path` | A directory, or a character vector of file paths. |
| `journal` | Registry id, for example "plos_one". |
| `column` | Which column width the figures target. Either one value for all files, or a named character vector mapping file name to column. |
| `dpi` | Resolution the files were written at. Useful for files that do not record it, such as those from base R's png() device. |
| `pattern` | Regular expression selecting files when path is a directory. Defaults to common figure extensions. |
...
| Option | What it does |
|---|---|
| `filename` | Output path. The extension selects the format. If it has no extension, the journal's first accepted format is used. |
| `plot` | Plot to save. Defaults to the last plot displayed. |
| `journal` | Registry id, for example "cell_press". |
| `column` | Which column width to size to. |
| `height` | Height. Defaults to three quarters of the width, which is a convenience, not a journal requirement. |
| `units` | Units for height. |
| `dpi` | Resolution. Defaults to the journal's stated minimum, or 300 when it states none. |
| `check` | Whether to check the result and report failures as a warning. |
