...
colour scale. Applying it while the plot is editable makes the
requirements part of the build. The specification may be an included
...
- style:
  ...
  Visual choices are applied first, followed by any overlapping
...
shape scale replaces the corresponding figspec scale, which lets you
make an explicit final choice when the project needs one.
